...
export default function CalendarPage() {
    const { data, loading, setData, page, setPage, limit, setLimit, total, setTotal, setFilters } = useDataFetcher("center/calendar", false);
    const columns = [
    ...
    const inputs = [
        { data: { id: "date", type: "date", label: "Date" }, pattern: { required: { value: true, message: "Please select a date" } } },
        { data: { id: "examType", type: "SelectField", label: "Exam Type", options: examTypes, loading: false }, pattern: { required: { value: true, message: "Please select an exam type" } } }
    ];

    return (
          <div>
              <div className={"grid grid-cols-2 gap-5 items-center my-2 bg-bgSecondary w-full py-2 px-2"}>
                  <FilterSelect options={filterExamTypes} label={"Exam Type"} onChange={handleExamTypeChange} loading={false} value={selectedExamType} />
              </div>
              <AdminTable
                    data={data}
                    columns={columns}
                    ...
                    extraComponent={({ item }) => (
                          <>
                          <ShiftAssignmentModal shifts={shifts} setFilters={setFilters} setData={setData}  item={item} label={"Attendees"} href="center/attendance"/>
                          </>
                    )}
              />
          </div>
